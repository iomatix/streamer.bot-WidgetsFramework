...
function UpdateAlertBox(platform, avatar, username, description, attribute, userForMessage, message, action, rawData) {
	alertQueue.push({
		platform,
		avatar,
		username,
...
function showClassicAlert(alert) {
	...
	container.innerHTML = ""; // czyścimy poprzedni alert
...
function showStackedAlert(alert) {
	...
	// 2‑frame delay to guarantee animation
	requestAnimationFrame(() => {
		requestAnimationFrame(() => {
